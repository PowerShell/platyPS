        ...
        foreach ($MamlFilePath in $Path)
        {
            ...
            try 
            {
                ...
                foreach ($command in $xml.helpItems.command.details.name)
                {
                    trap
                    {
                        Write-Error ("`nError occured trying to use cmdlet generated by provided MAML (New-Module Cmdelt). 
                        `nThis can be caused by schema invalid MAML being rejected by the PowerShell Help Engine.`n" +
                         $_.Exception.Message + "`n`n" + $_.InvocationInfo.ScriptName + "`n`n")
                        continue
                    } 
                    $thisDefinition = @"
