    ...
    process {
        ...
        function processMamlObjectToFile {
            ...
            process {
                ...
                $cf = ConvertMamlModelToConfluence -mamlCommand $mamlObject -metadata $newMetadata -NoMetadata:$NoMetadata

                MySetContent -path (Join-Path $OutputFolder "$commandName.md") -value $md -Encoding $Encoding -Force:$Force
                MySetContent -path (Join-Path $OutputFolder "$commandName.confluence") -value $cf -Encoding $Encoding -Force:$Force
            }
        }
...
function ConvertMamlModelToConfluence {
    param(
        [ValidateNotNullOrEmpty()]
        [Parameter(Mandatory = $true)]
        [Markdown.MAML.Model.MAML.MamlCommand]$mamlCommand,
        ...
        # Raiden: I wanted default to be "off" for this function, because confluence does not really need yaml
    ...
    begin {
        $parseMode = GetParserMode -PreserveFormatting:$PreserveFormatting
        $r = New-Object Markdown.MAML.Renderer.ConfluenceRenderer -ArgumentList $parseMode
        $count = 0
    }

    process {
        if (($count++) -eq 0 -and ($IncludeMetadata)) {
            return $r.MamlModelToString($mamlCommand, $metadata)
        }
        else {
            return $r.MamlModelToString($mamlCommand, $true) # skip version header
